	...
	class Log {
		...
		static function setupLogFiles($logPath) {
			...
			if (($dbLog = $logPath->touch("db.log")) === false) {
				$this->die("Db log file could not be created. Please make sure the log directory is writable.");
			}
			if (($errorLog = $logPath->touch("error.log")) === false) {
				$this->die("Error log file could not be created. Please make sure the log directory is writable.");
			}
			if (($infoLog = $logPath->touch("info.log")) === false) {
				$this->die("Info log file could not be created. Please make sure the log directory is writable.");
			}
